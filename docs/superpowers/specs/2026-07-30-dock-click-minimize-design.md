...
- **알려진 대가: 프론트모스트 앱의 아이콘은 드래그로 재배열할 수 없다.** mouseDown이 Dock에 닿지 않기 때문이다. 드래그를 살리려면 mouseDown을 통과시켜야 하는데 그러면 Dock이 우리 최소화를 되돌린다 — 양립할 수 없다. 다른 아이콘의 드래그는 영향받지 않는다
- 삼킨 mouseDown의 짝이 되는 mouseUp이 끝내 오지 않을 수 있으므로(제스처 도중 탭 비활성화 등), 삼킴 플래그에 1초 유효 기간을 둔다. 그러지 않으면 한참 뒤의 무관한 클릭이 삼켜진다
...
- **TCC 권한 변화의 즉시 반영 여부.** 실측에서 권한을 부여해도 실행 중이던 프로세스의 `auth_value`는 `1`(unknown)로 남았고 재시작 후에야 `2`(allowed)가 되었다. 회수 방향은 측정하지 못했다. `PermissionsManager`는 어느 쪽이든 동작하도록 만들고, 탭 생성이 실패하면 재실행 안내를 표시한다
